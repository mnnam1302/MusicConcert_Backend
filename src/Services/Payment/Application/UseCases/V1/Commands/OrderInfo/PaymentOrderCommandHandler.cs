...
public class PaymentOrderCommandHandler : ICommandHandler<Command.PaymentOrderCommand>
{
    ...
    public PaymentOrderCommandHandler(
    ...
    public async Task<Result> Handle(Command.PaymentOrderCommand request, CancellationToken cancellationToken)
    {
        /*
            1. check order existing
            2. get invoice for this Order -> ensure status is 'Pending'
            3. check invoice belong customer
        ...
        if (foundInvoice.Status != InvoiceStatus.Pending)
        ...
        // 4.
        foundInvoice.PaymentProcessedInvoice(request.TransactionCode);
